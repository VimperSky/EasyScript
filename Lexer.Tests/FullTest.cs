...
namespace Lexer.Tests
{
    public class FullTest
    {
        ...
        public void Default()
        {
            ...
            Assert.Equal(new Token(TokenType.OpenBracket, "(", 6, 7).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.AnyString, "Even", 6, 8).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.CloseBracket, ")", 6, 14).ToString(), lexer.GetNextToken().ToString());
            ...
            Assert.Equal(new Token(TokenType.OpenBracket, "(", 8, 21).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.CloseBracket, ")", 8, 22).ToString(), lexer.GetNextToken().ToString());
            ...
            Assert.Equal(new Token(TokenType.OpenBracket, "(", 11, 4).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Identifier, "_str", 11, 5).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.CloseBracket, ")", 11, 9).ToString(), lexer.GetNextToken().ToString());
