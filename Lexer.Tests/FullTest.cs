...
namespace Lexer.Tests
{
    public class FullTest
    {
        ...
        public void Default()
        {
            ...
            Assert.Equal(new Token(TokenType.KeyWord, "int", 0, 9).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Identifier, "a", 0, 13).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Comma, ",", 0, 14).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.KeyWord, "int", 0, 16).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Identifier, "br", 0, 20).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.CloseBracket, ")", 0, 22).ToString(), lexer.GetNextToken().ToString());
            ...
            Assert.Equal(new Token(TokenType.DivOp, "/", 5, 8).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Int, "2", 5, 10).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Equals, "==", 5, 12).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Int, "0", 5, 15).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.CloseBracket, ")", 5, 16).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.OpenBrace, "{", 5, 18).ToString(), lexer.GetNextToken().ToString());
            ...
            Assert.Equal(new Token(TokenType.CloseBracket, ")", 6, 15).ToString(), lexer.GetNextToken().ToString());
            Assert.Equal(new Token(TokenType.Semicolon, ";", 6, 16).ToString(), lexer.GetNextToken().ToString());
            ...
            Assert.Equal(new Token(TokenType.Identifier, "firstNumber", 8, 4).ToString(), lexer.GetNextToken().ToString());
